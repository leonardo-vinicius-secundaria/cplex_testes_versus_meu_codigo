...
// Reduced costs persistentes. O dual nao e recalculado nos restarts: apenas
// o limiar dependente do UB muda.
double storedReducedCost[MAXROW];
int totalRowsV2 = 0;
int availableRowsV2 = 0;
bool restartRequested = false;
int predictedFixings = 0;
int restartCount = 0;
int totalRestartFixings = 0;
int restartUB[MAX_RESTARTS];
int restartFixings[MAX_RESTARTS];
int lastPotentialFixings = 0;
int lastMaterialThreshold = 0;
...
int countNewFixingsForCurrentUB()
{
    int count = 0;
    for (int row = 1; row <= totalRowsV2; row++)
    {
        if (rowEliminada[row]) continue;
        count += limiteNaoPodeMelhorar(
            lowerBoundRaiz + storedReducedCost[row]
        );
    }
    return count;
...
void considerMaterialRestart()
{
    if (restartCount >= MAX_RESTARTS) return;

    const int newlyFixable = countNewFixingsForCurrentUB();
    const int minimumMaterial = max(
        16,
        (availableRowsV2 * 8 + 99) / 100
    );
    lastPotentialFixings = newlyFixable;
    lastMaterialThreshold = minimumMaterial;
    if (newlyFixable < minimumMaterial) return;

    predictedFixings = newlyFixable;
    restartRequested = true;
}

bool interruptCurrentSearch()
{
    return limiteTempoAtingido || restartRequested;
}
}

// ============================================================
// searchV2() — mesmo DLX da V1, acrescido somente do pedido de restart.
// ============================================================
void searchV2(int k, int MAX_K, int custoAtual)
{
    if (interruptCurrentSearch()) return;
    ...
    for (int index = inicioCandidatos; index < fimCandidatos; index++)
    {
        if (interruptCurrentSearch()) break;
...
int main(int argc, char* argv[])
{
    ...
    do
    {
        ...
        if (!restartRequested || limiteTempoAtingido) break;

        if (solSize != 0
            || topoArenaCandidatos != 0
            || topoArenaColunasCobertas != 0)
        {
            throw runtime_error("Estado DLX nao restaurado antes do restart");
        }

        const int fixedNow = applyStoredReducedCostFixing(instance.setCount);
        if (fixedNow <= 0 || fixedNow < predictedFixings)
            throw runtime_error("Fixing inconsistente no restart");

        restartUB[restartCount] = bestCost;
        restartFixings[restartCount] = fixedNow;
        restartCount++;
        totalRestartFixings += fixedNow;

        for (int column = 1; column <= nCols; column++)
        {
            colAtiva[column] = true;
            leftN[column] = column - 1;
            rightN[column] = column + 1;
        }
        leftN[header] = nCols;
        rightN[header] = 1;
        rightN[nCols] = header;
        lowerBoundAtivo = lowerBoundRaiz;
        fill(usedInBranch, usedInBranch + instance.setCount + 1, false);
        fill(forbiddenRow, forbiddenRow + instance.setCount + 1, false);
    }
    while (!limiteTempoAtingido);
    ...
    cout << "  Restarts materiais      : " << restartCount << "\n";
    cout << "  Fixings nos restarts    : " << totalRestartFixings << "\n";
    ...
    cout << "  Fixaveis no ultimo UB   : " << lastPotentialFixings << "\n";
    cout << "  Gatilho material        : " << lastMaterialThreshold << "\n";
