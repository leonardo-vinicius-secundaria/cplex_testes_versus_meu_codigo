/*
 * DLX V2 sem restart + bound residual de empacotamento de duas colunas.
 *
...
unsigned long long packingSeen[MAXCOL];
unsigned long long packingEpoch = 0;
long long totalPackingPrunes = 0;
...
void searchV2(int k, int MAX_K, int custoAtual)
{
    ...
    const unsigned long long currentPackingEpoch = ++packingEpoch;
    packingSeen[c] = currentPackingEpoch;
    int dynamicMinimumCost = INT_MAX;
    for (int node = downN[c]; node != c; node = downN[node])
    {
        ...
        dynamicMinimumCost = min(dynamicMinimumCost, rowCost[row]);
        ...
        for (int j = rightN[node]; j != node; j = rightN[j])
        {
            ...
            packingSeen[colID[j]] = currentPackingEpoch;
            novasCoberturas++;
    ...
    if (fimCandidatos == inicioCandidatos)
    {
        totalPodas++;
        totalPodasColunaSemLinha++;
        return;
    ...
    // Se nenhuma linha disponivel cobre simultaneamente c e d, as duas
    // colunas exigem linhas distintas. A soma de seus menores custos e um
    // lower bound valido para o residual; escolhemos o d que mais o fortalece.
    int incompatibleMinimumCost = 0;
    for (int column = rightN[header]; column != header; column = rightN[column])
    {
        if (packingSeen[column] == currentPackingEpoch) continue;
        if (menorCustoColuna[column] == INT_MAX) continue;
        incompatibleMinimumCost = max(
            incompatibleMinimumCost,
            menorCustoColuna[column]
        );
    }

    const double packingBound = static_cast<double>(dynamicMinimumCost)
                              + incompatibleMinimumCost;
    const double strengthenedResidual = max(lowerBoundAtivo, packingBound);
    if (limiteNaoPodeMelhorar(custoAtual + strengthenedResidual))
    {
        totalPodas++;
        totalPackingPrunes++;
...
int main(int argc, char* argv[])
{
    ...
    cout << "\nInstancia carregada — DLX V2 sem restart + bound residual!\n";
    ...
    cout << "Modo                     : Set Cover / DLX V2 sem restart + bound residual\n\n";
    ...
    cout << "  Podas por packing 2-col : " << totalPackingPrunes << "\n";
