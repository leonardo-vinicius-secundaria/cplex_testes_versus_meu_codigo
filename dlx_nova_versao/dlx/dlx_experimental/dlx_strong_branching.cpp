/*
 * DLX V2 sem restart + strong branching leve para escolha da coluna.
 *
...
#define chooseColumn chooseColumnV1NaoUsado
#include "../dlx.cpp"
#undef chooseColumn
#undef main
#undef search
...
constexpr int STRONG_COLUMN_SHORTLIST = 4;
...
long long strongColumnsEvaluated = 0;
long long strongCandidatesScanned = 0;
...
int chooseColumnStrong(int custoAtual)
{
    int shortlist[STRONG_COLUMN_SHORTLIST];
    int shortlistSize = 0;

    for (int column = rightN[header]; column != header; column = rightN[column])
    {
        if (shortlistSize == STRONG_COLUMN_SHORTLIST
            && colSize[column] >= colSize[shortlist[shortlistSize - 1]])
        {
            continue;
        }

        int position = shortlistSize;
        if (position < STRONG_COLUMN_SHORTLIST)
            shortlistSize++;
        else
            position = STRONG_COLUMN_SHORTLIST - 1;

        while (position > 0
               && colSize[column] < colSize[shortlist[position - 1]])
        {
            if (position < STRONG_COLUMN_SHORTLIST)
                shortlist[position] = shortlist[position - 1];
            position--;
        }

        if (position < STRONG_COLUMN_SHORTLIST)
            shortlist[position] = column;
    }

    int bestColumn = shortlist[0];
    int bestSurvivors = INT_MAX;

    for (int position = 0; position < shortlistSize; position++)
    {
        const int column = shortlist[position];
        int survivors = 0;
        strongColumnsEvaluated++;

        for (int node = downN[column]; node != column; node = downN[node])
        {
            const int row = rowID[node];
            if (rowEliminada[row] || forbiddenRow[row] || usedInBranch[row])
                continue;

            strongCandidatesScanned++;
            double coveredDual = pesoDual[column];
            for (int j = rightN[node]; j != node; j = rightN[j])
            {
                if (colunaAtiva(colID[j]))
                    coveredDual += pesoDual[colID[j]];
            }

            const double childBound = custoAtual
                                    + rowCost[row]
                                    + lowerBoundAtivo
                                    - coveredDual;
            if (!limiteNaoPodeMelhorar(childBound)
                && custoAtual + rowCost[row] < bestCost)
            {
                survivors++;
            }
        }

        const bool better = survivors < bestSurvivors
            || (survivors == bestSurvivors
                && colSize[column] < colSize[bestColumn])
            || (survivors == bestSurvivors
                && colSize[column] == colSize[bestColumn]
                && menorCustoColuna[column] > menorCustoColuna[bestColumn])
            || (survivors == bestSurvivors
                && colSize[column] == colSize[bestColumn]
                && menorCustoColuna[column] == menorCustoColuna[bestColumn]
                && pesoDual[column] > pesoDual[bestColumn]);
...
void searchV2(int k, int MAX_K, int custoAtual)
{
    ...
    int c = chooseColumnStrong(custoAtual);
    if (colSize[c] == 0)
    {
        totalPodas++;
...
int main(int argc, char* argv[])
{
    ...
    cout << "\nInstancia carregada — DLX V2 sem restart + strong branching!\n";
    ...
    cout << "Modo                     : Set Cover / DLX V2 sem restart + strong branching\n\n";
    ...
    cout << "  Colunas strong avaliadas: " << strongColumnsEvaluated << "\n";
    cout << "  Candidatos strong lidos : " << strongCandidatesScanned << "\n";
