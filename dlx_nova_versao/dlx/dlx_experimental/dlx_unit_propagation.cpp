/*
 * DLX V2 sem restart + propagacao de candidato unico.
 *
...
long long totalUnitPropagations = 0;
...
void searchV2(int k, int MAX_K, int custoAtual)
{
    ...
    // Uma coluna com uma unica linha disponivel torna essa linha obrigatoria
    // no ramo. Executa diretamente o filho, sem criar/ordenar candidatos nem
    // manter a estrutura de ramos irmaos, pois eles nao existem.
    if (colSize[c] == 1)
    {
        int forcedNode = -1;
        ...
        const int forcedRow = rowID[forcedNode];
        const int newCost = custoAtual + rowCost[forcedRow];
        const double childBound = newCost + lowerBoundAtivo - coveredDual;
        ...
        totalUnitPropagations++;
        usedInBranch[forcedRow] = true;
        solution[solSize++] = forcedNode;

        const double lowerBoundBeforeChild = lowerBoundAtivo;
        const int coveredColumnsStart = topoArenaColunasCobertas;
        j = forcedNode;
        do
        {
            const int column = colID[j];
            if (colunaAtiva(column))
            {
                if (topoArenaColunasCobertas >= MAXCOL)
                    throw runtime_error("Arena de colunas cobertas insuficiente");
                colSize[column]--;
                cover(column);
                arenaColunasCobertas[topoArenaColunasCobertas++] = column;
            }
            j = rightN[j];
        }
        while (j != forcedNode);

        searchV2(k + 1, MAX_K, newCost);

        for (int index = topoArenaColunasCobertas - 1;
             index >= coveredColumnsStart;
             index--)
        {
            uncover(arenaColunasCobertas[index]);
            colSize[arenaColunasCobertas[index]]++;
        }
        topoArenaColunasCobertas = coveredColumnsStart;
        lowerBoundAtivo = lowerBoundBeforeChild;
        solSize--;
        usedInBranch[forcedRow] = false;
        return;
    }
...
int main(int argc, char* argv[])
{
    ...
    cout << "\nInstancia carregada — DLX V2 sem restart + propagacao unitaria!\n";
    ...
    cout << "Modo                     : Set Cover / DLX V2 sem restart + propagacao unitaria\n\n";
    ...
    cout << "  Propagacoes unitarias   : " << totalUnitPropagations << "\n";
