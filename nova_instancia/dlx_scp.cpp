...
//  Testado com scpa1.txt  (300 elementos, 3000 conjuntos)
//
...
//       podava ramos da solução ótima em algumas instâncias.
...
double       TIME_LIMIT_SEC = 18000.0; // configurável via argumento
const long long LOG_EVERY  = 100000000LL;
const long long TIME_CHECK_EVERY = 1000000LL;
...
//
//  Checa o tempo com frequencia maior que a impressao de progresso.
//  Assim o benchmark nao precisa matar o processo externamente antes
//  de o DLX imprimir o resultado final. O B&B
...
{
    if (totalNos % TIME_CHECK_EVERY != 0)
        return;
...
void search(int k, int MAX_K, int custoAtual)
{
    ...
    if (candidatos.size() >= 2 && candidatos.size() <= 64)
                   ...
                   vector<int>& bestSolOut)
{
    ...
    if (!bestSolOut.empty() && u != nullptr)
    {
        ...
        int ruinReps = max(20, nIters / 2);
        int maxRemove = max(10, min(25, (int)bestSolOut.size() / 3));

        for (int rep = 0; rep < ruinReps; rep++)
        {
            int K = 3 + (int)(uniform_int_distribution<int>(0, maxRemove - 3)(rng));
...
// ============================================================
//  localSearch2opt — busca local combinada
//
//  Para cada conjunto j na solução, tenta substitui-lo por um
//  conjunto k tal que (sol \ {j}) ∪ {k} ainda cobre tudo e
//  cost[k] < cost[j]. Se não melhorar, tenta trocas pequenas
//  2->1 e 1->2, que são importantes quando a melhoria depende
//  de uma combinação de conjuntos e não de uma troca isolada.
//
                    ...
                    vector<int>& sol)
{
    ...
    while (improved && iters < 50)
    {
        ...
        if (!improved)
        {
            // 2->1: remove dois conjuntos e tenta colocar um conjunto
            // mais barato que cubra todos os elementos descobertos.
            for (size_t a = 0; a < sol.size() && !improved; a++)
            {
                for (size_t b = a + 1; b < sol.size() && !improved; b++)
                {
                    int ja = sol[a];
                    int jb = sol[b];
                    int removedCost = inst.cost[ja] + inst.cost[jb];

                    vector<int> touched;
                    touched.reserve(inst.coverElems[ja].size() + inst.coverElems[jb].size());
                    for (int e : inst.coverElems[ja]) { cov[e]--; touched.push_back(e); }
                    for (int e : inst.coverElems[jb]) { cov[e]--; touched.push_back(e); }

                    sort(touched.begin(), touched.end());
                    touched.erase(unique(touched.begin(), touched.end()), touched.end());

                    vector<int> uncov;
                    uncov.reserve(touched.size());
                    for (int e : touched)
                        if (cov[e] == 0) uncov.push_back(e);
                    ...
                    {
                        if (b > a) {
                            sol.erase(sol.begin() + b);
                            sol.erase(sol.begin() + a);
                        } else {
                            sol.erase(sol.begin() + a);
                            sol.erase(sol.begin() + b);
                        }
                    ...
                    {
                        int bestK = -1;
                        int bestCost = removedCost;
                        for (int k = 1; k <= n; k++)
                        {
                            if (!alive[k] || inSol[k] || k == ja || k == jb) continue;
                            if (inst.cost[k] >= bestCost) continue;

                            bool coversAll = true;
                            const auto& ce = inst.coverElems[k];
                            for (int e : uncov)
                                if (!binary_search(ce.begin(), ce.end(), e))
                                {
                                    coversAll = false;
                                    break;
                                }
                    ...
                    if (!improved)
                    {
                        for (int e : inst.coverElems[jb]) cov[e]++;
                        for (int e : inst.coverElems[ja]) cov[e]++;
                    }
                }
            }
        }
        ...
        if (!improved)
        {
            // 1->2: remove um conjunto caro e tenta cobrir o buraco
            // com dois conjuntos mais baratos em conjunto.
            for (size_t pos = 0; pos < sol.size() && !improved; pos++)
            {
                int j = sol[pos];
                int costJ = inst.cost[j];
                ...
                vector<int> uncov;
                uncov.reserve(inst.coverElems[j].size());
                for (int e : inst.coverElems[j])
                    if (cov[e] == 0) uncov.push_back(e);
                ...
                else if (uncov.size() <= 63)
                {
                    uint64_t fullMask = (uncov.size() == 64)
                        ? ~0ULL
                        : ((1ULL << uncov.size()) - 1ULL);
                    ...
                    for (int k = 1; k <= n; k++)
                    {
                        if (!alive[k] || inSol[k] || k == j) continue;
                        if (inst.cost[k] >= costJ) continue;

                        uint64_t mask = 0;
                        const auto& ce = inst.coverElems[k];
                        for (size_t uidx = 0; uidx < uncov.size(); uidx++)
                            if (binary_search(ce.begin(), ce.end(), uncov[uidx]))
                                mask |= (1ULL << uidx);
                        if (mask != 0)
                            cands.push_back({k, inst.cost[k], mask});
                    }

                    int bestA = -1, bestB = -1;
                    int bestPairCost = costJ;
                    for (size_t a = 0; a < cands.size(); a++)
                    {
                        for (size_t b = a + 1; b < cands.size(); b++)
                        {
                            int pairCost = cands[a].cost + cands[b].cost;
                            if (pairCost >= bestPairCost) continue;
                            if ((cands[a].mask | cands[b].mask) != fullMask) continue;
                            bestA = cands[a].id;
                            bestB = cands[b].id;
...
int main(int argc, char* argv[])
{
    ...
    // Argumentos: <inst> <time_limit> [--seed K] [--workers N] [--quiet] [--stop-on-feasible]
    ...
    for (size_t i = 0; i < args.size(); i++)
    {
        ...
        else if (args[i] == "--stop-on-feasible") stopOnFeasible = true;
    }
        ...
        for (int w = 0; w < workers; w++) {
            ths.emplace_back([&, w]() {
                string cmd = exePath + " \"" + caminho + "\" "
                             ...
                             + " --seed " + to_string(w) + " --quiet"
                             + (stopOnFeasible ? " --stop-on-feasible" : "")
                             + " 2>&1";
                FILE* p = popen(cmd.c_str(), "r");
                if (!p) return;
                {
    ...
    //  mas em instâncias pequenas fica próximo da melhor UB conhecida.
    ...
    int igUB = iteratedGreedy(inst, alive, u_opt, 1000, bestSolPool);
    ...
    // A UB final costuma ser melhor que a UB usada no primeiro RCFIX.
    // Repetir o reduced-cost fixing com essa UB mais apertada pode remover
    // mais conjuntos antes de montar a DLX e reduzir a árvore de B&B.
    int rcf2 = reducedCostFixing(inst, alive, u_opt, LB_lag, greedyUB);
    cout << "[RCFIX-2] Conjuntos eliminados apos UB final: " << rcf2 << "\n";
    ...
    if (stopOnFeasible && bestSolSize > 0)
    {
        double tWallTotal = gTimer.elapsedWall();
        double tCPUTotal  = gTimer.elapsedCPU();

        cout << "[STOP-ON-FEASIBLE] Solucao viavel encontrada; encerrando antes do B&B.\n";
        cout << "\n=== RESULTADO FINAL ===\n";
        cout << "Custo minimo : " << bestCost << "\n";
        cout << "Num. conjuntos: " << bestSolSize << "\n";
        cout << "Conjuntos escolhidos: ";
        for (int i = 0; i < bestSolSize; i++)
            cout << bestSol[i] << " ";
        cout << "\n[AVISO] Solucao viavel encontrada; otimalidade nao foi provada neste modo.\n";
        cout << "\nEstatisticas de busca:\n";
